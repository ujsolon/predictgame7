...
5. **Logo chips:** white/paper rounded chips behind logos on the ink OG field; "no container shape" rule relaxed to off-ink surfaces. **Approved provisionally — owner wants to see the revised `key-og-card.html` mock (with Miami/Spurs) before it sticks.** → **CONFIRMED 2026-09-25 against the revised mock; plus owner addition: historic-variant context moves from eyebrow to the center slot between the team blocks (custom variant keeps eyebrow + VS). Spines and mock synced; provisional flag lifted.**
