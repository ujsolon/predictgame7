...
- **OG card** — fixed 1200×630 canvas. Two horizontal zones: upper field (a matchup row of two team blocks — logo on a rounded white chip + abbreviation in mono — around a center slot: the historic card puts the context there, "2016 FINALS" over "GAME 7", with no top eyebrow; the custom card puts "VS" there under a "CUSTOM MATCHUP" eyebrow; **no series score, ever**), lower band (wordmark left, tagline "Where data meets playoff drama" right, hairline rule above). Margins 64px on each canvas side; type sizes (owner-approved 2026-09-25, as mocked in `mockups/key-og-card.html`): wordmark 48px, team abbreviations 64px mono, center context 30px with "GAME 7" 20px in `#8A8A8A`.
...
- **OG card** (`@vercel/og`, server-rendered PNG per AD-6) — ink field, two team blocks (each logo on a rounded white `logo-chip` so dark logo art — Miami, San Antonio — never vanishes into the field), logos as the only color, wordmark + tagline band. Three variants, one template: historic series (year · round · "GAME 7" context in the center slot between the team blocks — no eyebrow — **no series score or winner**), custom matchup ("CUSTOM MATCHUP" eyebrow, "VS" center, no year, no series score — never prediction outputs), fallback (wordmark + tagline only, no teams). When the shared link carries a method (`?method=<slug>`), the lower band adds a label-style method line ("MODEL: {METHOD}") left of the tagline — Story 4.2; owner-approved placement, re-tunable once seen against the real UI. Rendered by the share-og Edge Function; never a client component. Companion `og:title`/`og:description` copy per variant lives in EXPERIENCE.md · Voice and Tone. *Chips + center-context placement confirmed by owner against the revised mock (2026-09-25) — call 5 of the sprint change proposal settled.*
