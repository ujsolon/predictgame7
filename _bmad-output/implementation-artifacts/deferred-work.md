...
- source_spec: `spec-1-1-toolchain-foundation.md`
  ...
  evidence: `ci.yml` steps were reproduced locally (lint / test / build all green on the final state), but the workflow itself, the Node 22 runner, and `npm ci` from a clean cache need the owner's first push to confirm.
- source_spec: `spec-1-1-toolchain-foundation.md`
